...
final public class Rule_minutes extends Rule
{
  public Rule_minutes(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_minutes parse(ParserContext context)
  {
    ...
    parsed = false;
    {
      ...
      if (parsed)
      {
        ...
        {
          Rule rule = Terminal_NumericValue.parse(context, "%x30-35", "[\\x30-\\x35]", 1);
          if ((f1 = rule != null))
          {
    ...
    if (parsed)
    {
        rule = new Rule_minutes(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
