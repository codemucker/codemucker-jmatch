	...
	public static class PropertyMatcherBuilder<T> {
		...
		private static class GetterMethodMatcher<T> extends AbstractNotNullMatcher<T>{
			...
			GetterMethodMatcher(Method m,Matcher propertyMatcher){
				this.getterMethod = Preconditions.checkNotNull(m,"null getter");
				this.propertyMatcher = Preconditions.checkNotNull(propertyMatcher,"null matcher");
			}
			...
			public void describeTo(Description desc) {
				...
				desc.value("getter", getterMethod.getName());
				desc.value("is", propertyMatcher);
			}
		}
	}
}
